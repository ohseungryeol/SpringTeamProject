    ...
    <div>
        ...
        <div style="text-align:center" class="mt-3">
            ...
            <div class="mt-1" v-if="bm==0">
                <input type="file" @change="registerImage" accept=".jpg, .png"></input>
            </div>
...
export default {
    ...
    created() {
        ...
        axios_contact({
            ...
            this.preImage = data.data.pic;
            this.billingNO=data.data.billingNo;

            this.deliveryDate = data.data.deliveryDate[0]+"-";
            if(data.data.deliveryDate[1]<10){
                this.deliveryDate+="0"
            }
            this.deliveryDate+=data.data.deliveryDate[1]+"-"
            if(data.data.deliveryDate[2]<10){
                this.deliveryDate+="0"
            }
            this.deliveryDate+=data.data.deliveryDate[2]
            
            this.receiptDate = data.data.receiptDate[0]+"-";
            if(data.data.receiptDate[1]<10){
                this.receiptDate+="0"
            }
            this.receiptDate+=data.data.receiptDate[1]+"-"
            if(data.data.receiptDate[2]<10){
                this.receiptDate+="0"
            }
            this.receiptDate+=data.data.receiptDate[2]
    ...
    methods: {
        ...
        registerPaidForm(){
            ...
            formData.append("billingNo", String(this.billingNO));
            formData.append("deliveryDate", this.deliveryDate);
            formData.append("receiptDate", this.receiptDate);
            if(this.updateOrNot){
                if(this.image==null){
                    axios_contact.put("/paidForm/"+that.partyId, formData, {
                        header: {
                    ...
                    axios_contact.post("/paidForm/"+that.partyId, formData).
                        then(({data})=>{
                          console.log(data)
                          this.$router.push("/partydetail/"+that.partyId)
                    });
